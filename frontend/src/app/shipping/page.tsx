      ...
      <div className="py-14 px-6" style={{ backgroundColor: '#EBEBCA', borderBottom: '1px solid #B68868' }}>
        <div className="max-w-3xl mx-auto text-center">
          <div className="w-12 h-12 flex items-center justify-center mx-auto mb-5"
            ...
            <Truck size={20} style={{ color: '#642308' }} strokeWidth={1.5} />
          </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-14 space-y-14">
          ...
          <div style={{ border: '1px solid #EBEBCA' }}>
            ...
            {shippingOptions.map((opt, idx) => (
              <div key={opt.name} className="grid grid-cols-4 gap-4 px-5 py-4 items-center"
                style={{ borderTop: idx > 0 ? '1px solid #EBEBCA' : 'none' }}>
                <p className="text-sm font-semibold" style={{ color: '#642308' }}>{opt.name}</p>
                <div className="flex items-center gap-2">
                  <Clock size={12} style={{ color: '#B68868', flexShrink: 0 }} />
                  <p className="text-sm" style={{ color: '#903E1D' }}>{opt.time}</p>
                </div>
                <p className="text-sm font-semibold" style={{ color: '#642308' }}>{opt.charge}</p>
                <p className="text-xs" style={{ color: '#B68868' }}>{opt.detail}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={title} className="p-6 text-center" style={{ border: '1px solid #EBEBCA', backgroundColor: '#ffffff' }}>
              <div className="w-10 h-10 flex items-center justify-center mx-auto mb-4"
                style={{ border: '1px solid #EBEBCA' }}>
                <Icon size={15} style={{ color: '#B68868' }} strokeWidth={1.5} />
              </div>
        ...
        {/* Policy details */}
        <section className="space-y-6" style={{ borderTop: '1px solid #EBEBCA', paddingTop: '3rem' }}>
          ...
          {[
            ...
            <div key={title} className="flex gap-6">
              <div className="w-0.5 flex-shrink-0" style={{ backgroundColor: '#EBEBCA' }} />
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: '#642308' }}>{title}</p>
                <p className="text-sm leading-relaxed" style={{ color: '#903E1D' }}>{content}</p>
              </div>
            </div>
          ))}
        </section>
        ...
        <section style={{ borderTop: '1px solid #EBEBCA', paddingTop: '3rem' }}>
          ...
          <div className="space-y-6">
            {faqs.map(({ q, a }) => (
              <div key={q} className="pb-6" style={{ borderBottom: '1px solid #EBEBCA' }}>
                <p className="text-sm font-bold mb-2" style={{ color: '#642308' }}>{q}</p>
                <p className="text-sm leading-relaxed" style={{ color: '#903E1D' }}>{a}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="text-center pt-2">
          <p className="text-sm mb-4" style={{ color: '#903E1D' }}>Questions about your shipment?</p>
          <Link href="/contact"
